...
instance Display Expression where
    display (ConstantExpression c) tab = replicate tab '\t' <> "ConstantExpression:\n" <> (display c (tab + 1))
    display (VariableExpression v) tab = replicate tab '\t' <> "VariableExpression:\n" <> (replicate (tab + 1) '\t') <> v <> "\n"
    display (CallExpresssion callee n) tab = replicate tab '\t' <> "Call:\n" <> (replicate (tab + 1) '\t') <> "Callee:\n" <> (display callee (tab + 2)) <> (replicate (tab + 1) '\t') <> "Arguments:\n" <> (intercalate "" (map (\e -> display e (tab + 2)) n))
    display (Procedure n a body) tab = replicate tab '\t' <> "Procedure:\n" <> (replicate (tab + 1) '\t') <> "Name:\n" <> (replicate (tab + 2) '\t') <> n <> "\n" <> (replicate (tab + 1) '\t') <> "Arguments: (" <> (intercalate "," a) <>")\n" <> (replicate (tab + 1) '\t') <> "Body:\n" <> (intercalate "\n" (map (\e -> display e (tab + 2)) body))
    display (Builtin n _ _) tab = replicate tab '\t' <> "Builtin:\n" <> (replicate (tab + 1) '\t') <> n
    display (Lambda a body) tab = replicate tab '\t' <> "Lambda:\n" <> (replicate (tab + 1) '\t') <> "Arg: (" <> (intercalate "," a) <>")" <> (replicate (tab + 1) '\t') <> "Body:\n" <> (intercalate "\n" (map (\e -> display e (tab + 2)) body))
    display (If t c a) tab = replicate tab '\t' <> "If\n" <> (display t (tab + 1)) <> (replicate tab '\t') <> "then\n" <> (display c (tab + 1)) <> (replicate tab '\t') <> "else\n" <> (display a (tab + 1))

instance Display Definition where
    display (Var n e) tab = replicate tab '\t' <> "Define:\n" <> (replicate (tab + 1) '\t') <> "Name: " <> n <> "\n" <> (replicate (tab + 1) '\t') <> "Value:\n" <> (display e (tab + 2))

instance Display Form where
    display (Def d) tab = replicate tab '\t' <> "Definition:\n" <> (display d (tab + 1))
    display (Expr d) tab = replicate tab '\t' <> "Expression:\n" <> (display d (tab + 1))
...
identifier = label "identifier" $ ((:) <$> initial <*> some char) <|> (\c -> [c]) <$> (letter <|> special <|> oneOf "+-")
...
procedure = label "procedure" $ (uncurry (,,) <$> call_pattern) <*> (some $ lexeme expression)
...
expression = cexpr <|> vexpr <|> lamb <|> cond <|> callexpr
    where
        ...
        callexpr = wrap $ CallExpresssion <$> (lexeme expression) <*> many (lexeme expression)
        lamb = wrap $ (lexeme (string "lambda")) *> (uncurry Lambda <$> lambda)
        cond = wrap $ If <$> ((lexeme (string "if")) *> (lexeme expression)) <*> (lexeme expression) <*> (lexeme expression)

_definition :: StringParser Definition
_definition = var <|> pro
    where
        var = promote <$> (lexeme (string "define") *> identifier) <*> (lexeme expression)
        pro = cast <$> (lexeme (string "define") *> (lexeme (uncurry3 Procedure <$> procedure)))
...
builtinEq lst s = (ConstantExpression . Boolean) <$> ((==) <$> (collect (lst!!0) s) <*> (collect (lst!!1) s))

builtinLower :: [Expression] -> LispState -> IO Expression
builtinLower lst s = low <$> (collect (lst!!0) s >>= convert) <*> (collect (lst!!1) s >>= convert)
    where
        low (ConstantExpression (Number a)) (ConstantExpression (Number b)) = (ConstantExpression . Boolean) $ a < b
        convert c@(ConstantExpression (Boolean True)) = fail $ show c <> " is not a real number"
        convert c@(ConstantExpression (Boolean False)) = fail $ show c <> " is not a real number"
...
builtinAdd lst s = add <$> (collect (lst!!0) s >>= convert) <*> (collect (lst!!1) s >>= convert)
    where
        add (ConstantExpression (Number a)) (ConstantExpression (Number b)) = (ConstantExpression . Number) $ a + b
        convert c@(ConstantExpression (Boolean _)) = fail $ show c <> " is not a number"
        convert c@(ConstantExpression _) = pure c
...
builtinSub lst s = sub <$> (collect (lst!!0) s >>= convert) <*> (collect (lst!!1) s >>= convert)
    where
        sub (ConstantExpression (Number a)) (ConstantExpression (Number b)) = (ConstantExpression . Number) $ a - b
        convert c@(ConstantExpression (Boolean _)) = fail $ show c <> " is not a number"
        convert c@(ConstantExpression _) = pure c
        convert p@(Procedure _ _ _) = fail $ show p <> " is not a number"
        convert b@(Builtin _ _ _) = fail $ show b <> " is not a number"
...
builtinMul lst s = mul <$> (collect (lst!!0) s >>= convert) <*> (collect (lst!!1) s >>= convert)
    where
        mul (ConstantExpression (Number a)) (ConstantExpression (Number b)) = (ConstantExpression . Number) $ a * b
        convert c@(ConstantExpression (Boolean _)) = fail $ show c <> " is not a number"
        convert c@(ConstantExpression _) = pure c
        convert p@(Procedure _ _ _) = fail $ show p <> " is not a number"
        convert b@(Builtin _ _ _) = fail $ show b <> " is not a number"
...
builtinDiv lst s = di <$> (collect (lst!!0) s >>= convert) <*> (collect (lst!!1) s >>= convert >>= pass)
    where
        di (ConstantExpression (Number a)) (ConstantExpression (Number b)) = (ConstantExpression . Number) $ div a b
        convert c@(ConstantExpression (Boolean _)) = fail $ show c <> " is not a number"
        convert c@(ConstantExpression _) = pure c
        convert p@(Procedure _ _ _) = fail $ show p <> " is not a number"
        convert b@(Builtin _ _ _) = fail $ show b <> " is not a number"
...
builtinMod lst s = mo <$> (collect (lst!!0) s >>= convert) <*> (collect (lst!!1) s >>= convert >>= pass)
    where
        mo (ConstantExpression (Number a)) (ConstantExpression (Number b)) = (ConstantExpression . Number) $ mod a b
        convert c@(ConstantExpression (Boolean _)) = fail $ show c <> " is not a number"
        convert c@(ConstantExpression _) = pure c
        convert p@(Procedure _ _ _) = fail $ show p <> " is not a number"
        convert b@(Builtin _ _ _) = fail $ show b <> " is not a number"
...
test (CallExpresssion e args) s = eval =<< (collect e s)
    where
        eval (Procedure _ a body) = (evaluateBody body =<< generateProcedureScope (zip a args) s) >>= sink
        eval (Builtin _ narg func) = guard (length args == narg) *> ((\ex -> collect ex s) =<< (func args s))  >>= sink
        eval (Lambda a body) = (evaluateBody body =<< generateProcedureScope (zip a args) s) >>= sink
...
collect (VariableExpression n) s@(LispState defs) = (chute =<< (mfromMaybe =<< searchDefinition n defs)) <|> fail ("variable " <> n <> " is not bound")
    where
        chute (Var _ val) = collect val s
collect (CallExpresssion e args) s@(LispState _) = eval =<< (collect e s)
    where
        eval (Procedure _ a body) = (evaluateBody body =<< generateProcedureScope (zip a args) s)
        eval (Builtin _ narg func) = guard (length args == narg) *> ((\ex -> collect ex s) =<< (func args s))
        eval (Lambda a body) = (evaluateBody body =<< generateProcedureScope (zip a args) s)
        eval ex = fail $ "attempt to apply non-procedure " <> show ex
collect (If t c a) s@(LispState _) = condition =<< (test t s)
    where
...
generateProcedureScope lst s@(LispState def) = (pure . LispState) =<< add =<< (mapM (uncurry coll) lst)
    where
...
evaluateExpressionInBody (VariableExpression n) s@(LispState defs) = (disp =<< (mfromMaybe =<< searchDefinition n defs)) <|> fail ("variable " <> n <> " is not bound")
    where
        disp (Var _ val) = evaluateExpressionInBody val s
evaluateExpressionInBody (CallExpresssion e args) s@(LispState _) = eval =<< (collect e s)
    where
        eval (Procedure _ a body) = (evaluateBody body =<< generateProcedureScope (zip a args) s) *> pure s
        eval (Builtin _ narg func) = guard (length args == narg) *> ((\ex -> evaluateExpressionInBody ex s) =<< (func args s)) *> pure s
        eval (Lambda a body) = (evaluateBody body =<< generateProcedureScope (zip a args) s) *> pure s
        eval ex = fail $ "attempt to apply non-procedure " <> show ex
evaluateExpressionInBody _ s = pure s

evaluateExpression :: Expression -> LispState -> IO LispState
evaluateExpression (VariableExpression n) s@(LispState defs) = (disp =<< (mfromMaybe =<< searchDefinition n defs)) <|> fail ("variable " <> n <> " is not bound")
    where
        disp (Var _ val) = evaluateExpression val s
evaluateExpression (CallExpresssion e args) s@(LispState _) = eval =<< (collect e s)
    where
        eval (Procedure _ a body) = ((evaluateBody body =<< generateProcedureScope (zip a args) s) >>= print) *> pure s
        eval (Builtin _ narg func) = guard (length args == narg) *> ((\ex -> evaluateExpression ex s) =<< (func args s)) *> pure s
        eval (Lambda a body) = ((evaluateBody body =<< generateProcedureScope (zip a args) s) >>= print) *> pure s
        eval ex = fail $ "attempt to apply non-procedure " <> show ex
evaluateExpression (If t c a) s = (condition =<< (sink =<< (collect t s))) *> pure s
    where
        sink ex = test ex s
...
evaluate (Def (Var n e)) s@(LispState lst) = LispState <$> (((Var n) <$> (collect e s)) >>= (\def -> addDefinition def lst))
...
lispFileInterpreter = (getContents >>= (pure . (runParse lispParse))) >>= result
    where
        result (Right forms) = () <$ lispInterpreter forms defaultState
        result (Left es) = (putStrLn . show) es

_lispCLI :: Bool -> LispState -> IO LispState
_lispCLI True state = putStr "debug> " *> (hFlush stdout) *> (getLine >>= queryLine)
    where
        queryLine ":quit" = _lispCLI False state
        queryLine ":q" = _lispCLI False state
        queryLine l = result $ runParse lispParse l

        result (Right forms) = putStrLn (intercalate "   " (map (\e -> display e 0) forms)) *> (_lispCLI True =<< handle =<< (exec forms))
        result (Left es) = _lispCLI True state <* (putStrLn . show) es

        exec forms = Control.Exception.try $ lispInterpreter forms state :: IO (Either SomeException LispState)

        handle (Left e) = print e *> pure state
        handle (Right s) = pure s
_lispCLI False state = putStr "> " *> (hFlush stdout) *> (getLine >>= queryLine)
    where
